...
-- Records of problem
-- ----------------------------
INSERT INTO `problem` VALUES ('1', '0', '贪心', '树结构', '难', '彩色丝带', 'Bat有一个彩色的丝带，它可以表示为一个只包含小写字母字符串，每个字母代表一个颜色。现在他想把丝带分成几部分，保证每个部分没有重复的颜色。请你告诉他划分彩带一共有多少种不同的方式。', '输入一个字符串，表示彩色的丝带。(字符串只包含小写字母，字符串长度<10^5)', '输出一个数字n，表示对答案取模后的数字。', 'abbc', '4', '由于数字会非常大，输出的答案对 1000000007 取余.', '2017-06-03 20:00:00', '1', '2');
INSERT INTO `problem` VALUES ('2', '0', '贪心', '树结构', '中等', 'Bat数', 'Bat从小喜欢奇特的东西，而且天生对数字特别敏感，一次偶然的机会，他发现了一个有趣的四位数2992，这个数，它的十进制数表示，其各位数字之和为2+9+9+2=22，它的十六进制数BB0，其各位数字之和也为22，同时它的十二进制数表示1894，其各位数字之和也为22，啊哈，真是巧啊。Bat非常喜欢这种四位数，由于他的发现，所以这里我们命名其为Bat数。但是要判断这样的数还是有点麻烦啊，那么现在请你帮忙来判断任何一个十进制的四位数，是不是Bat数吧。', '输入一个四位十进制数 n', '若n为Bat数，输出”YES”,否则输出”NO”', '2992', 'YES', '对于2992 ：   \\n十进制数为：  2992  其各位数字之和为2+9+9+2  =22\\n十六进制数为：BB0   其各位数字之和为B+B+0     =22\\n十二进制数为：1894  其各位数字之和为1+8+9+4 =22', '2017-06-03 20:00:00', '1', '2');
INSERT INTO `problem` VALUES ('3', '0', '贪心', '树结构', '中等', '叔叔的农场', 'Bat的叔叔最近新开了一个农场，准备用来养牛。已知农场第一年有一头母牛，它每年年初生一头小母牛。每头小母牛从第四个年头开始，每年年初也生一头小母牛。请帮Bat计算出第n年的时候，共有多少头母牛？', '输入一个数字n', '输出一个数代表第n年母牛的个数', '5', '6', '(n<=50)\\n前五年母牛个数如下：  \\n第一年母牛的个数：  1\\n第二年母牛的个数：  2\\n第三年母牛的个数：  3  \\n第四年母牛的个数：  4    \\n第五年母牛的个数：  6', '2017-06-03 20:00:00', '1', '2');
INSERT INTO `problem` VALUES ('4', '0', '贪心', '树结构', '简单', '热爱汉字', 'Bat是一个美籍华人，他对汉字特别感兴趣。他从叔叔那里得到了一段字符串，出于对汉字的热爱，他想给每个汉字后面加一个标记。对于输入的字符串，查找其中的汉字，在该汉字后面插入字符串“(love)”。', '输入一个字符串，只包含汉字和英文字符', '输出插入“(love)”后的字符串', '阿里bat百度腾讯', '阿(love)里(love)bat百(love)度(love)腾(love)讯(love)', '（不包含空格，字符串的长度<1000）\\n该环境下为utf-8编码，该编码下 ， 一个汉字=3个字节 ， 且这三个字节ASCLL码值都为负值。如 :\\n\"腾\" 存储在s[0]、s[1]、s[2]三个字节中。\\n满足条件：s[0]<0&&s[1]<0&&s[2]<0', '2017-06-03 20:00:00', '1', '2');
INSERT INTO `problem` VALUES ('5', '0', '贪心', '树结构', '简单', '最大素数因子', '今天Bat突发奇想，要找出整数N的最大素数因子 ，但他不知道该怎么做，于是向你求助。', '输入一个数字N', '输出N的最大素数因子', '10', '5', '(2<=N<=10^3)\\n10的因子有1、2、5、10，其最大的素数因子为5', '2017-06-03 20:00:00', '1', '2');
INSERT INTO `problem` VALUES ('6', '0', '贪心', '树结构', '简单', '字符串统计', 'Bat是一个淘气的小朋友，有一天他得到了一个字符串，他想统计一下字符串中的小写字母(‘a’-’z’)的个数，快来帮帮他吧。', '输入一个字符串', '输出一个整数，代表小写字母的个数。', 'AabcBB', '3', '字符串中不包含空格、字符串长度<10000', '2017-06-03 20:00:00', '1', '2');
